...
#
# CC-CEDICT 渲染引擎
# by firefly(dance.of.firefly@gmail.com)
# v1.6
# 2015-10-08 17:08
#
...
def downloadAndExtract():
    ...
    zip_path = ''.join([os.getcwd(), path.sep, ZIP_NAME])
    #将打包的文件解压 
    zf = zipfile.ZipFile(zip_path, 'r')  
    print('---开始解压数据文件---')
    zf.extract(DATA_NAME, os.getcwd())
    zf.close()
    data_path = ''.join([os.getcwd(), path.sep, DATA_NAME])
...
def ParseAndRendering(file_name):
    ...
    #---创建SQL数据库文件
    sql_db_path = ''.join([os.getcwd(), path.sep, SQL_DB_NAME])
    ...
    #---创建MDX源文件
    mdx_path = ''.join([os.getcwd(), path.sep, MDX_NAME])
...
def formatItem(han_simp, han_trad, pin, defi, pre_han):
    ...
    for d in defi :
        d = d.strip()
        # '状元[zhuang4 yuan2]' -> '狀元|状元[zhuang4 yuan2]'
        pattern1 = re.compile(r'(?P<before>[^\u4e00-\u9fa5\|a-zA-Z1-5]+)(?P<han>[a-zA-Z1-5\u4e00-\u9fa5]+)\[(?P<pinyin>[a-zA-Z1-5\s]+)\]')
        d = pattern1.sub(r'\g<before>\g<han>|\g<han>[\g<pinyin>]', d)    

        res = ""
        split_res = re.split(r'([^\s\|]+(?:\|[^\[\s]+)(?:\[[a-zA-Z1-5\s]+\]*))', d)
        pattern2 =  re.compile(r'(?P<t_han>[a-zA-Z1-5\u4e00-\u9fa5]+)(?:\|(?P<s_han>[a-zA-Z1-5\u4e00-\u9fa5]+))((\[(?P<pinyin>[a-zA-Z1-5\s]+)\])?)')
        for s in split_res :
            if s!=None :
                sresult = re.search(pattern2, s)
                if sresult :
                    ...
                    # jump to other entry
                    if HAN_SIMP_KEY :
                        s = pattern2.sub(r'<a href="entry://\g<s_han>">\g<s_han></a>'+ddpy, s)
                    else :
                        s = pattern2.sub(r'<a href="entry://\g<t_han>">\g<t_han></a>'+ddpy, s)
            else :
                continue
            res +=s
        i_dy += '<li>'+res+'</li>'
